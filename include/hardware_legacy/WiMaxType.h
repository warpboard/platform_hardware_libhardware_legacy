...
#ifndef _WIMAX_TYPES_
#define _WIMAX_TYPES_
...
typedef struct _WIMAX_API_PROFILE_INFO
{
	...
	CHAR               profileName[MAX_SIZE_OF_STRING_BUFFER]; /**< profile name. The profile name is the name from teh OMA-DM tree.*/ 
...
/// 0 == Mandatory contact node for the operatorís Technical Support contact information. 
/// 1 == Mandatory contact node for operatorís Subscription Portal contact information. 
...
/// The statuses provided by this API can generally be mapped to movements along the SDK common state machine. The indications may provide further detailed information using the APIís arguments when relevant.
